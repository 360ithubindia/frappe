...
def login(user, pwd, ip_addr=None, login_source=None, device=None):
    # Check if mobile_number is enabled
    if frappe.db.get_single_value("System Settings", "mobile_number"):
        # Perform your mobile number login logic here
        # For example, send OTP to the user's email
        send_otp_to_email(user)

        # Return a response indicating that OTP has been sent
        return {"message": _("OTP has been sent to your email. Please use it to log in.")}

    # If mobile_number is not enabled, proceed with the default login logic
    # Your existing login logic here...

# Example function to send OTP to the user's email
def send_otp_to_email(user):
    # Generate OTP (you may use your own logic to generate OTP)
    otp = generate_otp()

    # Save OTP in the database or use any other method to validate later
    frappe.cache().hset("login_otp:{0}".format(user), "otp", otp)
    frappe.cache().hset("login_otp:{0}".format(user), "expiry_time", add_to_date(get_datetime(), seconds=600))
...
# Example function to generate OTP
def generate_otp():
